...
function s.initial_effect(c)
    ...
    --Gain ATK/DEF based on opponent's field
...
--------------------------------------------------
--Material restriction: cannot be used as material except for DIVINE monsters
function s.matlimit(e,c,sumtype,tp)
    if not c then return false end
    return not c:IsAttribute(ATTRIBUTE_DIVINE)
end

--Gain ATK/DEF based on number of cards opponent controls
...
function s.immval(e,te,c)
    if not c:IsPosition(POS_FACEUP) then return false end
...
--Reset other cards' effects on this card at End Phase
...
--Función que manda todas las cartas del campo al GY
function s.erasop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if not c:IsLocation(LOCATION_GRAVE) then return end   -- aseguramos que la carta esté en GY
    Duel.Hint(HINT_CARD,tp,id)                           -- muestra la carta que activó el efecto
    Duel.Hint(HINT_CARD,1-tp,id)
    local g=Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)  -- todas las cartas en el campo
    if #g>0 then
