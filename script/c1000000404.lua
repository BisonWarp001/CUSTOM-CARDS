...
function s.initial_effect(c)
	--Must be Special Summoned (from your Extra Deck) by sending 1 "The Wicked Avatar" + 1 "Divine Evolution"
	c:EnableReviveLimit()
	...
	--Prevent cards/effects activation when Special Summoned + Auto Destroy opponent's Spell/Traps
	...
	--ATK/DEF = Highest ATK on the field +100 (except The Wicked Avatar or self)
	...
	--Quick Effect: negate activation and destroy that card
	...
	--When leaves field: Special Summon 1 "The Wicked Avatar" or add 1 "Divine Evolution"
	local e10=Effect.CreateEffect(c)
	e10:SetDescription(aux.Stringid(id,1))
	e10:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_TOHAND+CATEGORY_SEARCH)
	e10:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e10:SetCode(EVENT_LEAVE_FIELD)
	e10:SetProperty(EFFECT_FLAG_DELAY)
	e10:SetCountLimit(1,{id,1})
	e10:SetTarget(s.lftg)
	e10:SetOperation(s.lfop)
...
function s.spfilter1(c)
	return c:IsCode(21208154) and c:IsAbleToGraveAsCost() -- The Wicked Avatar
end
function s.spfilter2(c)
	return c:IsCode(7373632) and c:IsAbleToGraveAsCost() -- Divine Evolution
end
...
------------------------------------------------------------
-- ATK/DEF calculation
function s.filteratk(c,e)
	return c:IsFaceup() and not (c:IsCode(21208154) or c:IsCode(id)) and c~=e:GetHandler()
end
function s.adval(e,c)
	local g=Duel.GetMatchingGroup(s.filteratk,0,LOCATION_MZONE,LOCATION_MZONE,nil,e)
	if #g==0 then return 100 end
...
------------------------------------------------------------
-- When leaves field: revive Wicked Avatar or add Divine Evolution
function s.revfilter(c,e,tp)
	return c:IsCode(21208154) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
